...
const SlipGrid = ({ setSlipNo,  handleApprovalSlip,slipList }) => {
  ...
  const approvalBtn =  () => {
    ...
    handleApprovalSlip(approvalData);
  ...
  const companionBtn =  () => {
    ...
    handleApprovalSlip(companionData);
  
    alert(` ${companionData.length} 건 의 전표가 반려 되었습니다. `);
    positionGridApi.updateRowData({ remove: selectedData });
      ...
      <div id="grid-wrapperr" style={{ width: "100%", height: "100%" }}>
        ...
        >
          <AgGridReact
            ...
            rowData={slipList} // setData된 state를 결국 여기 넣어서 그리드에 표현함.
